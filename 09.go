...
func main() {
	...
	ring := make([]Line, 0, len(points)-1)
	for i := 0; i < len(points)-1; i++ {
		ring = append(ring, LineFromPoints(points[i], points[i+1]))
	}

	verticalLines := sortedLines(ring, map[Dir]bool{UP: true, DOWN: true})
	horizontalLines := sortedLines(ring, map[Dir]bool{LEFT: true, RIGHT: true})

	startDir := opposite(verticalLines[0].Dir)

	startLines := sortedLines(ring, map[Dir]bool{startDir: true})
	oppositeLines := sortedLines(ring, map[Dir]bool{opposite(startDir): true})
	...
	// the _opposite_ line we're casting too, they'll be shadowed, but points along these lines
	...
	for i := 0; i < len(points)-1; i++ {
		for j := len(points) - 1; j > i; j-- {
			...
			// Early exit if we already found a larger part2
			if area <= part2 {
				continue
			}
